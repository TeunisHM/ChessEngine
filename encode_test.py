...
import random
import torch
from helper import move_to_index, index_to_move, board_to_tensor
...
def test_board_tensor_symmetry(board):
    ...
    # Make a mirrored board from opponent's perspective
    flipped_board = board.mirror()  # returns a new Board object
...
def run_tests(num_random_positions=100):
    ...
    for i in range(num_random_positions):
        ...
        # 1) Move encoding reversibility
        errs = test_move_encoding_on_board(board)
        if errs:
            print(f"  Move encoding errors in random board {i+1}")
            all_move_errors.extend(errs)

        # 2) Board-to-tensor symmetry
        if not test_board_tensor_symmetry(board):
            print(f"  Symmetry test FAILED in random board {i+1}")
            symmetry_failures += 1

    # Summary
    if not all_move_errors and symmetry_failures == 0:
        print("\n✅ All tests passed.")
    else:
        print(f"\n❌ Found {len(all_move_errors)} move encoding errors "
              f"and {symmetry_failures} symmetry failures.")
